...
#  COIN_VER=$(echo $COIN_ZIP | awk -F'/' '{print $NF}' | sed -n 's/.*\([0-9]\.[0-9][0-9]\.[0-9]\).*/\1/p')
  ...
  tar xvfj $COIN_ZIP --strip=1 "${COIN_DIR}/${COIN_NAME,,}d" "${COIN_DIR}/${COIN_NAME,,}-tx" "${COIN_DIR}/${COIN_NAME,,}-cli" # >/dev/null 2>&1
  compile_error
...
ExecStart=$COIN_DAEMON -daemon -conf=$CONFIGFOLDER/$CONFIG_FILE -datadir=$CONFIGFOLDER -debug -logips=1 -forcestart -rescan
